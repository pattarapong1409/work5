...
router.post("/insert", async (req, res) => {
    ...
    let pids : number;
    let sql = mysql.format("select pid from Person where name = ?",[person.pid])
    ...
    let mids : number;
    sql = mysql.format("select mid from Movie where name = ?",[person.mid])
    ...
    sql = "INSERT INTO `stars`( `mid`, `pid`) VALUES (?,?)";
  ...
  router.delete("/deletebyid/:pid/:mid", (req, res) => {
    ...
    conn.query("delete from stars where mid = ? and pid = ?", [mid,pid], (err, result) => {
       if (err) throw err;
       res
         .status(200)
         .json({ affected_row: result.affectedRows });
